...
- F2-T05 foi concluída e validada humanamente pelo Champion: RED, GREEN e rollback aprovados em 2026-09-18; o GREEN foi retestado e aprovado em 2026-09-21 após correção do detector de qualidade (Skip v0.0.78).
- A entrega ficou restrita à collection `decisoes_f2` (migration 0020), hook exclusivo, seção Decisão de Marketing no detalhe do experimento e fila `/decisoes-f2`.
- Rollback provado: `DEC-F2-004` com `previous_decision_id = DEC-F2-002`; `DEC-F2-002` preservada como `revogada` com motivo; nada apagado.
- Transparência: `DEC-F2-003` foi criada durante o teste pelo botão genérico, sem vínculo com a cadeia de rollback; permanece na massa sintética, sem exclusão ou alteração.
- TDD determinístico 6/6 na versão final; QA completo aprovado; regressões Fase 1 (10×10, diferença 0), F2-T01/T02 (IN/OUT/RED) e F2-T04 (lote 17, pipeline 10) preservadas.
- Sem Meta, RD Station, 1CRM, Omie, publicação, alteração de orçamento, atribuição real ou relação estrutural `demandas` ↔ `experimentos_f2`.
- F2-T06 e F2-T07 não foram iniciadas; ambas elegíveis na ordem, uma task por vez, mediante autorização expressa.
