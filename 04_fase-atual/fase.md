...
- As SPECs F2-001 e F2-002 foram substituídas pelas versões validadas da mesma decomposição, eliminando a divergência de numeração CA/RN entre esta jornada e as SPECs — divergência que estava registrada como aberta desde a F2-T03.
...
## Registro de fechamento da F2-T07 (2026-09-21)

- O Champion aprovou humanamente o `META-F2-001`, a modalidade `fallback_manual` e o contrato documental da task.
- A aprovação confirmou `batch_id` como chave de idempotência do lote manual; identificadores de campanha/anúncio como atributos; `record_id` restrito à fonte declarada; ausência de identidade global/multi-fonte; tratamento de não vinculados, desconhecidos e divergências; métricas de campanha, com `spend` somente como leitura; exclusão de leads, formulários, contatos e dados pessoais; e ausência de token, OAuth e chamada Meta.
- Os papéis operacionais e de qualidade/reconciliação permanecem definidos por função nesta etapa; owner operacional nominal deverá ser informado quando houver operação real.
- Revalidação independente confirmou: documentação completa; RED equivalente bloqueado sem acesso/credencial/payload; GREEN documental aprovado; Skip v0.0.81 (`e64f8bc`) preservado; migrations até `0020_f2_t05_decisoes`; collections e rotas existentes preservadas; `demandas`, T04 e decisões F2 preservadas; nenhuma chamada externa ou dado real.
- O arquivo de verificador previsto pela rotina não estava disponível neste runtime; checklist equivalente foi executado em série e a limitação foi registrada no relatório.
...
- F2-T08 foi liberada como próxima task e permanece aguardando teste humano; F2-T09 permanece condicional e nenhuma task posterior foi iniciada.

## Registro de implementação da F2-T08 (2026-09-21)

- A F2-T08 foi implementada conforme autorização expressa do Champion, sem iniciar a F2-T09.
- O núcleo puro de reconciliação foi generalizado em `src/lib/f2/reconciliation/core.ts`; `src/lib/f2/t04/engine.ts` permanece como adapter compatível da T04.
- O adapter manual, o contrato de lote, `META-F2-MANUAL-001`, o ledger em memória, `batch_id`, replay e conflito de payload foram implementados em `src/lib/f2/t08/manualBatch.ts`.
- A superfície de prova reutiliza a rota existente `/atribuicao-t04`, em seção visual explicitamente rotulada como prova sintética T08; não foi criada nova página isolada.
- O lote é 100% sintético, com registros vinculado, não vinculado, desconhecido, divergente e inválido. Primeira execução é dry-run; `demandas` permanece com 10 registros.
- Replay idêntico do mesmo `batch_id` resulta em `replay_skip` com zero novas ações. O mesmo `batch_id` com payload diferente resulta em `payload_conflict`, `bloqueada` e decisão humana necessária.
- 401, 403, 429, timeout e payload inválido são eventos injetados e explicitamente marcados como simulados; não são respostas reais do Meta, não há retry automático e não houve chamada externa.
- TDD determinístico T08 passou 8/8; regressão T04 passou 7/7; QA oficial da versão Skip 0.0.85 (`25c717a`) passou em setup, análise estática, build, integrações e testes.
- Não houve nova collection, migration, campo, hook, RLS, escrita em `demandas`, token, OAuth, dado real, dado pessoal, publicação ou alteração de orçamento.
- Estado atual: implementação concluída; aguardando teste humano do Champion. A F2-T08 não está formalmente concluída.
